...
FitThreadFast::FitThreadFast(int _iterations, int _thread_id, int _n_threads, double** _data, double _length)
{
	SYSTEMTIME t;
	GetLocalTime(&t);
	...
	contents.myInt = new IntegratorFast(0,0,0,0,0,0,0,0);
...
UINT FitThreadFast::calculateMinT()
{
	{		
			...
			for(int i = 0; i <contents.iterations; i++)
		{
			...
			contents.myInt->changeParams(F,dF1,dF2,phi,A,T,Td,m);
...
double FitThreadFast::randomDouble(double min, double max)
{				
	double randNum = min + (max - min)*( (double) (*dist)(*gen)/((double)RAND_MAX+1.0));
	return randNum;
}

double FitThreadFast::calculateResidual()
{
	double residual = 0;
	...
	{		
		...
		double val = contents.myInt->integrateLegendre(data[i][0],0,-PI,PI);
		residual += (val - data[i][1])*(val - data[i][1]);
	}		
	return residual;
}

FitThreadFast::~FitThreadFast(void)
{
}

UINT FitThreadFast::startThread(LPVOID param)
{
	FitThreadFast* _this = reinterpret_cast<FitThreadFast *>(param);	
