#include "StdAfx.h"
#include "RandomFitThreadedFast.h"
...
RandomFitThreadedFast::RandomFitThreadedFast(double ** _data_set,int _length, int _n_threads, int _iterations)
{
	...
	p_threads = new FitThreadFast*[n_threads];


}

RandomFitThreadedFast::~RandomFitThreadedFast(void)
{
	delete [] p_threads;
}

void RandomFitThreadedFast::setThreadNumber(int _n_threads)
{
	delete [] p_threads;
	n_threads = _n_threads;
	p_threads = new FitThreadFast*[n_threads];
...
void RandomFitThreadedFast::setIterations(int _iterations)
{
	iterations = _iterations;
}
...
double RandomFitThreadedFast::getMinimum()
{
	...
	for(int i = 0; i<n_threads; i++)
	{
		...
		p_threads[i] = new FitThreadFast(thread_iterations,i,n_threads,data_set,length);
...
void RandomFitThreadedFast::outputParams()
{
